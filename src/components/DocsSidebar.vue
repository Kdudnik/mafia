...
<template>
    <nav class="sticky top-52 right-0">
        <div
            ...
            <template v-for="(block, index) in data.sidebar">
                <div v-if="hasTranslation(index)">
                    <h4 class="duration-200 hover:text-gray-light">
                        <router-link :to="`${block.link}${block.anchor}`">{{ $t(`sidebar[${index}].title`) }}</router-link>
                    </h4>
                    <ul class="ml-4 text-gray-dark dark:text-white">
                        <li v-for="(topic, i) in block.topics">
                            <a class="duration-200 hover:text-gray-light" :href="`${block.link}${topic.anchor}`">{{
                                $t(`sidebar[${index}].topics[${i}].title`) }}</a>
                        </li>
                    </ul>
                </div>
            </template>
